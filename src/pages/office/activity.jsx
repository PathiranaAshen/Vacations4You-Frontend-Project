...
function Activity() {
  ...
  const getData = () => {
    axios
      .get(`http://localhost:5000/office/get/activity`)
      .then((res) => {
  ...
  function handleViewActivity(data) {
    Swal.fire({
      title: "Activity Details",
      html: `<div class='text-start'><br/>
                <p><strong>Activity Name:</strong> ${data.name}</p>
  ...
  function handleDeleteActivity(id) {
    Swal.fire({
      title: "Are you sure?",
      text: "You are about to delete this Activity. This action cannot be undone!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:5000/office/delete/activity/${id}`)
          .then(() => {
  ...
  async function search_activity() {
    await axios
      .get(`http://localhost:5000/office/search/activity/${searchText}`)
      .then((response) => {
        if (response.data) {
  ...
  const handleUpdateSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/office/update/activity/${values._id}`,
        values
      );
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/office/add/activity",
        formData,
        {
          headers: {
                  ...
                  {activityData ? (
                    activityData.map((act, index) => (
                      <tr key={index}>
                        ...
                        <td className="text-center">{act.type}</td>
